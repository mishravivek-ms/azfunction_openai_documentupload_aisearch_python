...
def move_blob(source_container_client, destination_container_client, source_blob, destination_blob_name):
    
    source_blob_delete = source_container_client.get_blob_client(source_blob.name.replace(container_name+"/", ""))
    destination_blob = destination_container_client.get_blob_client(destination_blob_name)
    
    destination_blob.start_copy_from_url(source_blob.uri)
    source_blob_delete.delete_blob()

def populate_index(blob):
    ...
    try:
        ...
        destination_blob_name=destination_blob_name.replace(container_name+"/", "")
        move_blob(container_client, container_client, blob, destination_blob_name)

        #----------------------------------------
        
        print(f"Successfully processed and deleted the source {blob.name}")
